...
conn = sqlite3.connect(DB_PATH, check_same_thread=False, timeout=10) # Added timeout
c = conn.cursor()

c.execute('''CREATE TABLE IF NOT EXISTS uploads (
    id INTEGER PRIMARY KEY AUTOINCREMENT, filename TEXT, uploader TEXT, timestamp TEXT,
    file_type TEXT, category TEXT, 
    submission_date TEXT, -- Stores the "Import From Date" selected during upload (effective date of the upload batch)
    file BLOB
)''')
...
category_file_types = {
    ...
    'missing': ['performance'],
    'visits': [], # No specific file types, category itself is the type
...
LOGO_PATH = "company_logo.png" # Ensure this file is in the same directory as your script
    ...
    if not st.session_state.authenticated:
        ...
        with st.form("login_form"):
            ...
            if submitted:
                ...
                elif username in view_only and password: # Password required for view_only
...
if is_admin:
    ...
    # Use a callback or ensure session state is up-to-date for dependent selectbox
    # For simplicity here, we'll read from session state directly in the button logic
    ...
    selected_file_type = st.sidebar.selectbox(
        ...
        disabled=(not valid_file_types), help="Options change based on category. Select category first." 
    )
    
    st.sidebar.markdown("**Filter Excel Data by Date Range for this Import:**")
    import_from_date_val = st.sidebar.date_input( # Renamed to avoid conflict with df column 'date'
        "Import Data From Date:", 
        value=date.today() - timedelta(days=7), 
        key="import_from_date_upload"
    )
    import_to_date_val = st.sidebar.date_input( # Renamed
        "Import Data To Date:", 
        value=date.today(), 
    ...
    if upload_btn: 
        # Read final values from session state at the time of button click
        final_category = st.session_state.admin_category_select
        final_file_type = st.session_state.admin_file_type_select
        imp_from_dt = st.session_state.import_from_date_upload # This is already a date object
        imp_to_dt = st.session_state.import_to_date_upload   # This is already a date object
        ...
        if requires_file_type and not final_file_type:
            ...
            if not requires_file_type: final_file_type = None # Set to None if category doesn't require a file_type
            
            data = up.getvalue()
            ts = datetime.now().isoformat()
            # submission_date for the uploads table now refers to the start of the import range from Excel
            # This helps identify the batch's intended coverage.
            upload_submission_date_str = imp_from_dt.isoformat() 
            
            # --- Start of single transaction attempt ---
            try:
                ...
                if c.fetchone()[0] > 0:
                    ...
                    # Proceed with Excel processing BEFORE inserting into 'uploads'
                    ...
                    if missing_cols:
                        ...
                        if len(df_excel_full) < original_excel_rows:
                            st.sidebar.warning(f"{original_excel_rows - len(df_excel_full)} Excel rows dropped due to invalid date format in Excel's 'date' column.")
                        
                        if df_excel_full.empty:
                            st.sidebar.error("No valid data rows with parsable dates found in Excel. Upload aborted.")
                        else:
                            # Filter Excel data based on the selected import date range
                            ...
                            if df_to_import.empty:
                                st.sidebar.info(f"No rows in '{up.name}' found within the selected import date range: {imp_from_dt:%Y-%m-%d} to {imp_to_dt:%Y-%m-%d}.")
                            else:
                                # If all checks pass and data is ready, THEN insert into 'uploads' and 'issues'
                                ...
                                for _, row in df_to_import.iterrows():
                                    issue_date_str = row['parsed_date'].strftime('%Y-%m-%d') # Date from Excel
                                ...
                                conn.commit() 
                                st.sidebar.success(f"Successfully imported {len(df_to_import)} issues from '{up.name}' for Excel dates {imp_from_dt:%Y-%m-%d} to {imp_to_dt:%Y-%m-%d}.")
                                st.rerun()
            except sqlite3.Error as e_sql: 
                conn.rollback() 
                st.sidebar.error(f"Database error during upload: {e_sql}. Upload rolled back.")
            except Exception as e_general: 
                conn.rollback() 
                st.sidebar.error(f"Error processing file '{up.name}': {e_general}. Upload rolled back if DB operations started.")
...
df_uploads_raw['display_submission_date_fmt'] = df_uploads_raw['submission_date'].apply(format_display_date) # Renamed for clarity

st.sidebar.subheader("Data Scope")
scope_opts = ['All uploads'] + [(f"{r['id']} - {r['filename']} ({r['category']}/{r['file_type'] or 'N/A'}) Imp.From: {r['display_submission_date_fmt']}") for i,r in df_uploads_raw.iterrows()]
sel_display = st.sidebar.selectbox("Select upload batch to analyze:", scope_opts, key="select_upload_scope") # Changed label
sel_id = int(sel_display.split(' - ')[0]) if sel_display != 'All uploads' else None

if is_admin:
    st.sidebar.subheader("Manage Submissions")
    delete_opts_list = [(f"{row['id']} - {row['filename']} ({row['category']}/{row['file_type'] or 'N/A'}) Imp.From: {row['display_submission_date_fmt']}") for index, row in df_uploads_raw.iterrows()]
    delete_opts = ['Select ID to Delete'] + delete_opts_list
    del_choice_display = st.sidebar.selectbox("🗑️ Delete Submission Batch:", delete_opts, key="delete_submission_id") # Changed label
...
# Fetch ALL data initially. `date` in issues table is the actual issue date from Excel.
...
sel_cat = st.sidebar.multiselect("Category (from Upload Batch):", cat_opts, default=['All'], key="category_filter") # Changed label
...
sel_ft = st.sidebar.multiselect("File Type (from Upload Batch):", file_type_filter_opts, default=['All'], key="file_type_filter") # Changed label
...
if enable_comparison:
    ...
    if safe_default_p1_end > max_overall_date: safe_default_p1_end = max_overall_date
    if min_overall_date > safe_default_p1_end : safe_default_p1_end = min_overall_date
    ...
    if comparison_date_range_1_val and len(comparison_date_range_1_val) == 2:
        ...
        if default_p2_start < min_overall_date: default_p2_start = min_overall_date
...
def apply_general_filters(df_input, sel_upload_id_val, selected_branches, selected_categories, selected_managers, selected_file_types):
    ...
    if sel_upload_id_val: df_filtered = df_filtered[df_filtered['upload_id_col'] == sel_upload_id_val] # Filter by selected upload batch
...
if df_primary_period.empty:
    ...
    if 'date' in df_primary_period.columns and pd.api.types.is_datetime64_any_dtype(df_primary_period['date']) and not df_primary_period['date'].isnull().all():
        ...
        if not trend_data_primary.empty:
            ...
            fig_trend.update_layout(title_text='Issues Trend (Primary Period - Based on Issue Dates)', xaxis_title='Date', yaxis_title='Number of Issues', template="plotly_white", hovermode="x unified", legend_title_text='Metric') # Clarified title
...
if enable_comparison and comparison_date_range_1 and comparison_date_range_2:
    st.markdown("---"); st.header("📊 Period Comparison Results (Based on Issue Dates)") # Clarified title
    df_comp1 = pd.DataFrame(columns=df_temp_filtered.columns) 
    df_comp2 = pd.DataFrame(columns=df_temp_filtered.columns)
    start_c1_disp, end_c1_disp, start_c2_disp, end_c2_disp = "N/A", "N/A", "N/A", "N/A" # For display if ranges are not set
