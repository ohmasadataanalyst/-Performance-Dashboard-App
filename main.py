...
if is_admin:
    ...
    if upload_btn:
        ...
        try:
            ...
            COMP_PERF_EXCEL_BRANCH_NAME_COL = 'branch' # From new schema: this column has branch name + code
            ...
            COMP_PERF_EXCEL_DATE_COL = 'date' # From new schema: this is the date column
...
# Ensure primary_date_range is always a list of two dates
if not primary_date_range_val or len(primary_date_range_val) != 2:
    ...
    primary_date_range = [primary_date_range_val[1], primary_date_range_val[0]] # Correct order
...
if enable_comparison: # ... (comparison setup unchanged)
...
def create_bar_chart(df_source, group_col, title_suffix="", chart_title=None):
    ...
    elif pd.api.types.is_numeric_dtype(df_valid_data[group_col]): # If it's numeric, no stripping needed
        pass # No stripping needed for numeric, dropna is enough
    else: # convert to string for others and then strip
    ...
    if not df_valid_data.empty:
        ...
        if not data.empty: return px.bar(data, x=group_col, y='count', title=final_title, template="plotly_white")
    return None

def create_pie_chart(df_source, group_col, title_suffix="", chart_title=None): # ... (similar update for pie chart)
    final_title = chart_title if chart_title else f"Issues by {group_col.replace('_',' ').title()} {title_suffix}"
    if group_col not in df_source.columns: return None
    df_valid_data = df_source.copy()
    df_valid_data.dropna(subset=[group_col], inplace=True)
    ...
    if not df_valid_data.empty:
        ...
        if not data.empty: return px.pie(data, names=group_col, values='count', title=final_title, hole=0.3, template="plotly_white")
    return None

# --- Complaint Details Parser ---
def parse_complaint_details(issue_string):
    ...
    if product_match:
        ...
        details['Product'] = product_val if product_val.lower() != "لا علاقة لها بالمنتج" else "N/A (Not Product Related)"
...
def display_complaints_performance_dashboard(df_complaints_raw):
    ...
    # Parse the 'issues' column into structured complaint details
    ...
    # Rename columns for clarity in charts if needed
    ...
    with col1:
        fig_type = create_bar_chart(df_complaints, 'Complaint Type', chart_title="Complaints by Type")
        if fig_type: st.plotly_chart(fig_type, use_container_width=True)
        
        fig_product = create_bar_chart(df_complaints, 'Product Complained About', chart_title="Complaints by Product")
        if fig_product: st.plotly_chart(fig_product, use_container_width=True)

    with col2:
        # Quality Issue Details (only for 'جوده' type complaints)
        df_quality_issues = df_complaints[df_complaints['Complaint Type'] == 'جوده']
        if not df_quality_issues.empty:
            fig_quality_detail = create_bar_chart(df_quality_issues, 'Quality Issue Detail', chart_title="Quality Issue Details (for 'جوده' complaints)")
            if fig_quality_detail: st.plotly_chart(fig_quality_detail, use_container_width=True)
        else:
            st.info("No 'جوده' (Quality) type complaints to detail.")

        # Order Error Details (only for 'خطاء فى الطلب' type complaints)
        df_order_errors = df_complaints[df_complaints['Complaint Type'] == 'خطاء فى الطلب']
        if not df_order_errors.empty:
            fig_order_error_detail = create_bar_chart(df_order_errors, 'Order Error Detail', chart_title="Order Error Details (for 'خطاء فى الطلب' complaints)")
            if fig_order_error_detail: st.plotly_chart(fig_order_error_detail, use_container_width=True)
        else:
            st.info("No 'خطاء فى الطلب' (Order Error) type complaints to detail.")
            
    st.subheader("Complaints by Branch")
    fig_branch_complaints = create_bar_chart(df_complaints, 'branch', chart_title="Total Complaints per Branch")
    ...
    if 'date' in df_complaints.columns and pd.api.types.is_datetime64_any_dtype(df_complaints['date']) and not df_complaints['date'].isnull().all():
        ...
        if not trend_data_complaints.empty:
            fig_complaints_trend = px.line(trend_data_complaints, x='date', y='daily_complaints', title='Daily Complaints Trend', markers=True)
...
figs_primary = {} # Initialize for general dashboard PDF figures

# Determine if the current view should be for complaints/performance
is_complaints_performance_data = False
if not df_primary_period.empty:
    # Check if ALL data in df_primary_period is complaints/performance
    # This is robust if filters are set to 'All' but underlying data is specific
    is_complaints_category = (df_primary_period['upload_category'] == 'complaints').all()
    is_performance_report = (df_primary_period['report_type'] == 'performance').all()
    
    # More flexible: check if ANY data is complaints/performance and if filters are specific
    # For simplicity, let's use the filter selection for now.
    # If 'All' is selected for category/type, this logic might show general dashboard even if data is pure complaints.
    # A more robust way is to check df_primary_period['upload_category'].nunique() == 1 etc.

    # Using explicit filter selection:
    sel_cat_list = [sel_cat] if isinstance(sel_cat, str) else sel_cat
    sel_ft_list = [sel_ft] if isinstance(sel_ft, str) else sel_ft

    if ('complaints' in sel_cat_list and len(sel_cat_list) == 1 and
        'performance' in sel_ft_list and len(sel_ft_list) == 1):
        is_complaints_performance_data = True
    # Also consider if 'All' is selected but the underlying data in df_primary_period is purely complaints/performance
    elif ('All' in sel_cat_list or not sel_cat_list) and ('All' in sel_ft_list or not sel_ft_list):
        # If filters are "All", check the data content
        if not df_primary_period.empty:
             if (df_primary_period['upload_category'] == 'complaints').all() and \
...
df_comp1, df_comp2 = pd.DataFrame(), pd.DataFrame() # ... (comparison logic unchanged from previous correct version)
if enable_comparison and comparison_date_range_1 and comparison_date_range_2:
    ...
    if df_comp1.empty and df_comp2.empty:
        ...
        # Determine if comparison data is also purely complaints/performance
        ...
        # If both periods are complaints/performance, show complaints-specific comparison
        if is_comp_complaints_data_p1 and is_comp_complaints_data_p2 :
            ...
            df_comp1_parsed.rename(columns={'Type': 'Complaint Type'}, inplace=True) # Focus on type for now
            ...
            if not df_combined_complaints_comp.empty:
                ...
                if not complaint_type_comp_data.empty:
                    fig_complaint_type_comp = px.bar(complaint_type_comp_data, x='Complaint Type', y='count', color='period_label', barmode='group', title='Complaint Types Comparison by Period')
                    st.plotly_chart(fig_complaint_type_comp, use_container_width=True)
        
        # Fallback to general comparison if not purely complaints or if one period is different
        else:
            ...
            # ... (rest of general comparison unchanged)
            ...
            st.markdown("#### Period-Level Trend (Average Daily Issues)") # ... (general trend unchanged)
...
# --- Downloads Section ---
st.sidebar.subheader("Downloads") # ... (downloads section mostly unchanged, PDF visuals are for general dashboard)
if 'df_primary_period' in locals() and not df_primary_period.empty:
    ...
    try:
        ...
        # If it's complaints data, we might want to save the parsed version
        if is_complaints_performance_data:
            parsed_details_dl = df_primary_period['issues'].apply(parse_complaint_details)
            df_primary_excel = pd.concat([df_primary_period.drop(columns=['issues']), parsed_details_dl], axis=1)
            ...
            df_primary_excel = df_primary_period
    ...
    # Visuals PDF is currently tied to `figs_primary` from the general dashboard
    if not is_complaints_performance_data: # Only offer general visuals PDF if not in complaints view
        if st.sidebar.button("Prepare Visuals PDF (Primary)", key="prep_visuals_pdf_primary"):
            if not wk_path or wk_path == "not found": st.sidebar.error("wkhtmltopdf path not set for PDF generation.")
                ...
                # ... (PDF generation for general dashboard visuals using figs_primary)
                with st.spinner("Generating Visuals PDF for Primary Period..."):
                    ...
                    chart_titles_in_order = ["Branch", "Area Manager", "Report Type", "Category", "Shift_Values", "Trend"] # Expected keys
    ...
    # Data Table PDF (can be made to show parsed complaints if in that view)
    if st.sidebar.button("Prepare Data Table PDF (Primary)", key="prep_dashboard_pdf_primary"): # ... (Data table PDF - can be adapted)
        if not wk_path or wk_path == "not found": st.sidebar.error("wkhtmltopdf path not set for PDF generation.")
        else:
            with st.spinner("Generating Data Table PDF for Primary Period..."):
                ...
                if is_complaints_performance_data:
                    ...
                    df_pdf_view_final = pd.concat([df_primary_period.drop(columns=['issues']), parsed_details_pdf], axis=1)
                    ...
                    df_pdf_view_final = df_pdf_view_final[pdf_table_cols]
                else:
                ...
                if pd.api.types.is_datetime64_any_dtype(df_pdf_view_final['date']): 
...
if enable_comparison and comparison_date_range_1 and comparison_date_range_2: # ... (comparison CSV unchanged)
