...
BRANCH_SCHEMA = {
    ...
    "B30": "QADRUH B30", "B31": "ANRUH B31", "B32": "FAYJED B32", "B33": "HIRJED B33", # Corrected FAYJED B32
    "B34": "URURUH B34", "B35": "IRRUH B35", # Corrected IRRUH B35
    "LB01": "Lubda Alaqeq Branch LB01", "LB02": "Alkhaleej Branch LB02", # Corrected Lubda Alaqeq
    "QB01": "As Suwaidi Branch QB01", "QB02": "Al Nargis Branch QB02", "TW01": "Twesste B01 TW01",
    # Added from HTML files if not present
    "FYJED B32": "FYJED B32" # This was in one HTML, ensure it's mapped if it's a distinct code/name
}
BRANCH_SCHEMA_NORMALIZED = {str(k).strip().upper(): v for k, v in BRANCH_SCHEMA.items()}
# Ensure all values from BRANCH_SCHEMA are also keys if they represent unique branch names not covered by codes
...
category_file_types = {
    ...
    'complaints': ['performance', 'اغلاق الشكاوي'], # Added 'اغلاق الشكاوي'
    'missing': ['performance'],
    'visits': [],
...
# Constants for 'اغلاق الشكاوي'
COMPLAINT_CLOSURE_EXCEL_AM_COL_NAME = 'مدير المنطقة المسؤول'
COMPLAINT_CLOSURE_EXCEL_STATUS_COL_NAME = 'مدى الاجراء المتخذ'
COMPLAINT_CLOSURE_EXCEL_DATE_COL_NAME = 'date submitted' # Ensure this matches the actual Excel header for date
...
with c1_mt:
    try: st.image(LOGO_PATH, width=100)
    except Exception as e: st.error(f"Logo error: {e}") # Gracefully handle if logo not found
    ...
    if up_btn:
        ...
        try:
            ...
            if norm_cat == 'cctv':
                ...
                elif norm_ft == CLOSURE_REPORT_TYPE_NAME.lower(): # 'اغلاق الشكاوي'
            ...
            for _, row_data in df_to_imp.iterrows():
                ...
                if norm_cat == 'complaints' and norm_ft == 'performance':
                    ...
                    iss_val = str(row_data.get(COMPLAINT_CLOSURE_EXCEL_STATUS_COL_NAME.lower(), "N/A")) # Status is the 'issue'
                elif norm_cat == 'cctv':
                ...
                if code_from_excel_col_direct and code_from_excel_col_direct in BRANCH_SCHEMA_NORMALIZED:
                    ...
                    for sc_code, sc_name in BRANCH_SCHEMA_NORMALIZED.items(): # Try full name match from BRANCH_SCHEMA values
                        if norm_excel_branch_name_str == sc_name.upper():
                            std_branch_name = sc_name
                            db_code_val = sc_code # sc_code is the key (original code)
                            match_found_by_full_name = True
                            break
                    if not match_found_by_full_name: # Also check if excel_branch_str is a key itself (e.g. "B01")
    ...
    if 'All' not in selected_file_types_val:
        ...
        if not valid_selected_ft: pass 
            ...
            if include_na_types:
                ...
                # Make sure to only keep rows that are NOT NA-like from the specifically filtered set
...
def display_complaint_closure_dashboard(df_closure_data, figs_container, period_name="Primary Period"):
    ...
    # --- Data Preparation ---
    def prepare_summary_df(df, group_by_col):
        ...
        summary[CLOSURE_COMPLETED_STATUS] = summary.get(CLOSURE_COMPLETED_STATUS, 0) # Ensure 'مكتمل' exists
        summary['نسبة الاكتمال'] = (summary[CLOSURE_COMPLETED_STATUS] / summary['المجموع'] * 100).fillna(0).round(1)
        
        # For display table: Count (Percentage%)
    ...
    # --- Tabs for Managers and Branches ---
    tab_managers, tab_branches = st.tabs(["📊 حسب المديرين", "🏢 حسب الفروع"])

    # Manager Analysis
    df_managers_raw_summary, df_managers_display = prepare_summary_df(df_closure_data, 'area_manager')
    with tab_managers:
        st.subheader("إحصائيات حسب المديرين")
        st.dataframe(df_managers_display, use_container_width=True)
    ...
    # Branch Analysis
    df_branches_raw_summary, df_branches_display = prepare_summary_df(df_closure_data, 'branch')
    with tab_branches:
        st.subheader("إحصائيات حسب الفروع")
        st.dataframe(df_branches_display.sort_values("المجموع", ascending=False), use_container_width=True)

        if not df_branches_raw_summary.empty:
            # Top N branches for chart
            top_n_branches = 15 # As in HTML example (or adjust)
            ...
            fig_branches = px.bar(df_branch_chart_data,
                                  barmode='stack',
                                  orientation='h', # Horizontal like HTML
    ...
    # --- Summary Section (like HTML example) ---
    ...
    if not df_branches_raw_summary.empty:
        ...
        df_low_completion_branches.sort_values('نسبة الاكتمال', ascending=True, inplace=True) # Sort by completion rate

        if not df_low_completion_branches.empty:
            # Prepare display table for low completion branches
            df_low_comp_display = pd.DataFrame(index=df_low_completion_branches.index)
            for status_col_lc in CLOSURE_STATUS_ORDER:
                 if status_col_lc in df_low_completion_branches.columns:
                    df_low_comp_display[status_col_lc] = df_low_completion_branches.apply(
                        lambda row: f"{int(row[status_col_lc])} ({((row[status_col_lc] / row['المجموع'] * 100) if row['المجموع'] > 0 else 0.0):.1f}%)", axis=1
                    )
                 else:
                    df_low_comp_display[status_col_lc] = "0 (0.0%)"
            df_low_comp_display['المجموع'] = df_low_completion_branches['المجموع'].astype(int)
            df_low_comp_display['نسبة الاكتمال'] = df_low_completion_branches['نسبة الاكتمال'].apply(lambda x: f"{x:.1f}%")
            
            # Rename index to 'الفرع' for display
            df_low_comp_display.index.name = "الفرع"
            st.dataframe(df_low_comp_display.reset_index(), use_container_width=True)
            
            # Chart for low completion branches
            ...
            figs_container['closure_low_completion_table_df'] = df_low_completion_branches # For PDF

            # Summary for low completion branches
            st.markdown("---")
            ...
            if total_non_completed_low_comp > 0:
                ...
                with st.container(): # Use columns for better layout if many statuses
                    ul_items = ""
...
figs_complaint_closure_primary = {} # For the new dashboard
...
if not df_primary_period.empty:
    ...
    if is_purely_complaint_closure:
        # st.subheader(f"تحليل {CLOSURE_REPORT_TYPE_NAME} (Primary Period)") # Title is inside the function
        figs_complaint_closure_primary = display_complaint_closure_dashboard(df_primary_period.copy(), figs_complaint_closure_primary, "Primary Period")
        ...
        if not df_complaint_closure_subset_in_primary.empty:
            st.markdown("---"); # Title is inside function
            temp_figs_subset_closure = display_complaint_closure_dashboard(df_complaint_closure_subset_in_primary, {}, "Subset of Primary")
    ...
    if st.button("🏆 Show Branch Rankings (Current Filters)", key="show_rankings_button_main_display"):
        if not df_primary_period.empty and 'branch' in df_primary_period.columns:
            ...
            elif is_purely_complaint_closure: # Ranking for 'اغلاق الشكاوي' based on 'نسبة الاكتمال'
                st.markdown("_Based on 'نسبة الاكتمال' (Completion Rate). Higher is better._")
                # Need to re-calculate raw summary for ranking if not already available outside display function
                # For simplicity, assume df_branches_raw_summary is available or re-calculate if necessary
                # This part might need adjustment if df_branches_raw_summary is not in scope here.
                # Let's assume it can be recalculated from df_primary_period for ranking.
                
                temp_df_for_rank, _ = display_complaint_closure_dashboard.__globals__['prepare_summary_df'](df_primary_period, 'branch') # Hacky way to call inner func; better to pass df or have separate logic
                
                if not temp_df_for_rank.empty and 'نسبة الاكتمال' in temp_df_for_rank.columns:
                    ranked_closure_df = temp_df_for_rank.sort_values(by='نسبة الاكتمال', ascending=False).copy()
                    ranked_closure_df['Rank'] = ranked_closure_df['نسبة الاكتمال'].rank(method='min', ascending=False).astype(int)
                    st.dataframe(ranked_closure_df[['Rank', 'نسبة الاكتمال', 'المجموع']].reset_index().rename(columns={'index':'branch', 'نسبة الاكتمال':'Completion Rate (%)', 'المجموع': 'Total Complaints'}), use_container_width=True)

                    if len(ranked_closure_df) > 1:
                        rank_chart_cols = st.columns(2)
                        top_n = min(10, len(ranked_closure_df))
                        with rank_chart_cols[0]:
                            fig_top_closure = create_bar_chart(ranked_closure_df.head(top_n), 'branch', chart_title=f"Top {top_n} (Highest Completion Rate)", sort_values_by='Completion Rate (%)', sort_ascending=False) # y value for sort is 'Completion Rate (%)'
                            if fig_top_closure: st.plotly_chart(fig_top_closure, use_container_width=True)
                        with rank_chart_cols[1]:
                            fig_bot_closure = create_bar_chart(ranked_closure_df.tail(top_n).sort_values('Completion Rate (%)',ascending=True), 'branch', chart_title=f"Bottom {top_n} (Lowest Completion Rate)", sort_values_by='Completion Rate (%)', sort_ascending=True)
                            if fig_bot_closure: st.plotly_chart(fig_bot_closure, use_container_width=True)
                else:
                    st.info("Could not generate ranking for complaint closure (missing completion rate data).")
...
if enable_comparison and comparison_date_range_1 and comparison_date_range_2:
    ...
    if df_comp1.empty and df_comp2.empty: st.info("No data for comparison in either period with the selected filters.")
        ...
        if not df_comp1_closure_data.empty or not df_comp2_closure_data.empty:
            ...
            if dfs_to_concat_closure_comp:
                ...
                if not df_combined_closure_comp.empty:
                    ...
                    with cl2_metric: st.metric(f"Total Records ({p2_lab})", total_cl2, delta=f"{delta_cl:+}" if delta_cl!=0 else None, delta_color="inverse" if delta_cl < 0 else "normal") # Assuming more records is not necessarily bad

                    closure_comp_chart_cols_c1, closure_comp_chart_cols_c2 = st.columns(2)
                    with closure_comp_chart_cols_c1: # Closure Status by Branch Comparison
                    ...
                    with closure_comp_chart_cols_c2: # Closure Status Comparison (Overall)
        ...
        # 4. General Issues Comparison (Excluding specialized complaint/missing/closure data)
...
is_primary_data_purely_complaint_closure_check = False # New check
...
if 'df_primary_period' in locals() and not df_primary_period.empty:
    ...
    try: 
        ...
        if is_primary_data_purely_complaints_check:
            ...
            excel_file_suffix = "complaint_closure_summary"
            # For Excel, export the raw summary data (not the display version)
            # This requires re-calculating or accessing the raw summary dataframes
            # For simplicity, we'll export the main df_primary_period and let user pivot if needed.
            # Or, if figs_complaint_closure_primary holds the DFs:
            # df_primary_excel_export = figs_complaint_closure_primary.get('closure_overall_summary_df', df_primary_period.copy())
            # A better approach for Excel export of closure data would be to export manager and branch summaries to separate sheets.
            # For now, exporting the filtered raw data for this type.
            df_primary_excel_export = df_primary_period.copy()
        ...
        if not wk_path or wk_path == "not found": st.sidebar.error("wkhtmltopdf path not set.")
            ...
            with st.spinner("Generating Visuals PDF..."):
                ...
                if active_pdf_visuals_type_key == "complaints_visuals_primary":
                    ...
                    ordered_keys_for_pdf = ['closure_managers_chart', 'closure_branches_chart', 'closure_low_completion_chart'] # Add more if needed
                else: 
    ...
    if st.sidebar.button("Prepare Data Table PDF (Primary)", key="prep_dashboard_table_pdf_primary"):
        if not wk_path or wk_path == "not found": st.sidebar.error("wkhtmltopdf path not set.")
            ...
            with st.spinner("Generating Data Table PDF..."):
                ...
                pdf_table_cols_display = ['date','branch','report_type','upload_category','issues','area_manager','code'] # Default
                ...
                if is_primary_data_purely_complaints_check: 
                    ...
                    # For PDF table, use the DFs stored in figs_complaint_closure_primary if they exist (e.g., summary tables)
                    # This part needs to be carefully constructed to mimic the multi-table layout
                    html_table_content_for_pdf = f"<h2>Summary by Managers</h2>"
                    # Recalculate or fetch df_managers_display logic
                    _, manager_disp_df_pdf = display_complaint_closure_dashboard.__globals__['prepare_summary_df'](df_pdf_final_table.rename(columns={'Status':'issues'}), 'area_manager')
                    ...
                    _, branch_disp_df_pdf = display_complaint_closure_dashboard.__globals__['prepare_summary_df'](df_pdf_final_table.rename(columns={'Status':'issues'}), 'branch')
                    ...
                    if 'closure_low_completion_table_df' in figs_complaint_closure_primary:
                        low_comp_df_pdf = figs_complaint_closure_primary['closure_low_completion_table_df']
                        # Prepare display version for low_comp_df_pdf
                        df_low_comp_display_pdf = pd.DataFrame(index=low_comp_df_pdf.index)
                        for status_col_lc_pdf in CLOSURE_STATUS_ORDER:
                            if status_col_lc_pdf in low_comp_df_pdf.columns:
                                df_low_comp_display_pdf[status_col_lc_pdf] = low_comp_df_pdf.apply(
                                    lambda row: f"{int(row[status_col_lc_pdf])} ({((row[status_col_lc_pdf] / row['المجموع'] * 100) if row['المجموع'] > 0 else 0.0):.1f}%)", axis=1)
                            else: df_low_comp_display_pdf[status_col_lc_pdf] = "0 (0.0%)"
                        df_low_comp_display_pdf['المجموع'] = low_comp_df_pdf['المجموع'].astype(int)
                        df_low_comp_display_pdf['نسبة الاكتمال'] = low_comp_df_pdf['نسبة الاكتمال'].apply(lambda x: f"{x:.1f}%")
                ...
                if not html_table_content_for_pdf: # If not already generated
                ...
                if is_primary_data_purely_missing_check and not df_missing_perf_results_primary.empty:
                    record_count_text = f"<p><strong>Branch Count in Summary:</strong> {len(df_missing_perf_results_primary)}</p>"
                     ...
                     # Add summary counts for closure report if available
