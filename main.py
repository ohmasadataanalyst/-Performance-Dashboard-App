...
c.execute('''CREATE TABLE IF NOT EXISTS uploads (
    ...
    submission_date TEXT,  -- Date representing the 'from_date' of the import batch
    file BLOB
)''')
c.execute('''CREATE TABLE IF NOT EXISTS issues (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    upload_id INTEGER, code TEXT, issues TEXT, branch TEXT, area_manager TEXT,
    date TEXT, -- Actual date of the issue/event from the Excel file
    report_type TEXT, -- Corresponds to 'file_type' from the upload
    shift TEXT, -- Specifically for CCTV shift times
    FOREIGN KEY(upload_id) REFERENCES uploads(id) ON DELETE CASCADE
)''')
conn.commit()

# Add 'submission_date' to 'uploads' if not exists
try:
...
# Add 'shift' to 'issues' if not exists
try:
...
db_admin = {
    ...
    "omarsalah": b"$2b$12$9VB3YRZRVe2RLxjO8FD3C.01ecj1cEShMAZGYbFCE3JdGagfaWomy", # Normalized key
    "ahmed khaled": b"$2b$12$wUMnrBOqsXlMLvFGkEn.b.xsK7Cl49iBCoLQHaUnTaPixjE7ByHEG",
    "mohamed hattab": b"$2b$12$X5hWO55U9Y0RobP9Mk7t3eW3AK.6uNajas8SkuxgY8zEwgY/bYIqe",
    "mohamedhattab": b"$2b$12$X5hWO55U9Y0RobP9Mk7t3eW3AK.6uNajas8SkuxgY8zEwgY/bYIqe"  # Normalized key
}
...
category_file_types = {
    ...
    'missing': ['performance'],
    'visits': [], # No specific file type, implies generic structure or to be handled
...
LOGO_PATH = "company_logo.png" # Ensure this file exists in the same directory or provide full path
...
def check_login():
    ...
    if not st.session_state.authenticated:
        col1_lgn, col2_lgn = st.columns([1,5]) # Ratio for logo and title
        with col1_lgn:
            try: st.image(LOGO_PATH, width=100)
            except Exception: pass # Logo is optional for login screen functionality
        with col2_lgn: st.title("📊 Login - Performance Dashboard")

        st.subheader("Please log in to continue")
        with st.form("login_form"):
            username_input = st.text_input("Full Name:", key="auth_username_login").strip() # Keep original input casing for display
            password = st.text_input("Password:", type="password", key="auth_password_login")
            submitted = st.form_submit_button("Login")

            if submitted:
                username_lower = username_input.lower() # Use lower for matching keys
                
                admin_hashed_pw = db_admin.get(username_lower) # Try direct lowercased match first
                if not admin_hashed_pw: # If not found, try removing spaces from lowercased username
                ...
                if admin_hashed_pw and bcrypt.checkpw(password.encode('utf-8'), admin_hashed_pw):
                    ...
                    st.session_state.user_name = username_input # Store original input for display
                    ...
                    st.rerun()
                elif username_lower in view_only and not password:
                    st.error("Password cannot be empty for view-only users.")
                elif username_input or password : # If any field was filled
...
if is_admin:
    ...
    if upload_btn:
        ...
        try:
            ...
            # --- Define Standardized Column Names Expected from Excel ---
            EXCEL_CODE_COL = 'code' 
            ...
            COMP_PERF_EXCEL_BRANCH_NAME_COL = 'branch' # This should be 'branch' based on sample
            ...
            for _, row in df_to_import.iterrows():
                ...
                if final_category == 'complaints' and final_file_type == 'performance':
                    ...
                    if pd.notna(complaint_type_val) and str(complaint_type_val).strip():
                        details.append(f"Type: {str(complaint_type_val).strip()}") # Commas will be preserved here
                    
                    product_str = str(product_val).strip() if pd.notna(product_val) else ""
                    if product_str: 
                        details.append(f"Product: {product_str}")
                    
                    if pd.notna(quality_val) and str(quality_val).strip():
                        details.append(f"Quality Detail: {str(quality_val).strip()}") # Commas preserved
                    
                    if pd.notna(order_error_val) and str(order_error_val).strip():
                        details.append(f"Order Error: {str(order_error_val).strip()}") # Commas preserved
    ...
    df_uploads_raw_for_delete['display_submission_date_fmt'] = df_uploads_raw_for_delete['submission_date'].apply(
        lambda d: datetime.strptime(str(d),'%Y-%m-%d').strftime('%Y-%m-%d') if pd.notna(d) and str(d) else "N/A"
    )
    
    delete_opts_list = [
        (f"{row['id']} - {row['filename']} ({row['category']}/{row['file_type'] or 'N/A'}) "
         f"Imp.From: {row['display_submission_date_fmt']}") for index, row in df_uploads_raw_for_delete.iterrows()
    ]
    ...
    if del_choice_display != 'Select ID to Delete':
        ...
        if st.sidebar.button(f"Confirm Delete Submission #{del_id_val}", key=f"confirm_del_btn_{del_id_val}", type="primary"):
            try:
                ...
                st.sidebar.success(f"Deleted submission batch {del_id_val}.")
                st.rerun()
...
wk_path = st.sidebar.text_input("wkhtmltopdf path (for PDF reports):", default_wk, help="Required for PDF generation. Example: /usr/local/bin/wkhtmltopdf or C:\\Program Files\\wkhtmltopdf\\bin\\wkhtmltopdf.exe")
...
scope_opts = ['All uploads'] + [
    (f"{r['id']} - {r['filename']} ({r['category']}/{r['file_type'] or 'N/A'}) "
     f"Imp.From: {r['display_submission_date_fmt']}") for i,r in df_uploads_raw_main.iterrows()
]
...
primary_date_range_val = st.sidebar.date_input(
...
filtered_report_types = sorted([rt for rt in unique_report_types if rt.lower() != 'none' and rt.lower() != 'nan'])
...
if enable_comparison:
    ...
    safe_default_p1_end = min(min_overall_date + timedelta(days=6), max_overall_date)
    safe_default_p1_end = max(safe_default_p1_end, min_overall_date) 

    comparison_date_range_1_val = st.sidebar.date_input(
        "Start & End Date (Period 1):",
        value=[min_overall_date, safe_default_p1_end],
        min_value=min_overall_date, max_value=max_overall_date, key="comparison_period1_filter"
    )
    if comparison_date_range_1_val and len(comparison_date_range_1_val) == 2 and comparison_date_range_1_val[0] <= comparison_date_range_1_val[1]:
        ...
        st.sidebar.warning("Period 1: Invalid date range.")
        comparison_date_range_1 = None 

    if comparison_date_range_1: 
        st.sidebar.markdown("Comparison Period 2 (Issue Dates):")
        default_p2_start = min(comparison_date_range_1[1] + timedelta(days=1), max_overall_date)
        default_p2_start = max(default_p2_start, min_overall_date) 

        default_p2_end = min(default_p2_start + timedelta(days=6), max_overall_date)
        default_p2_end = max(default_p2_end, default_p2_start) 

        comparison_date_range_2_val = st.sidebar.date_input(
            "Start & End Date (Period 2):",
            value=[default_p2_start, default_p2_end],
            min_value=min_overall_date, max_value=max_overall_date, key="comparison_period2_filter"
        )
        if comparison_date_range_2_val and len(comparison_date_range_2_val) == 2 and comparison_date_range_2_val[0] <= comparison_date_range_2_val[1]:
            ...
            st.sidebar.warning("Period 2: Invalid date range.")
...
def apply_general_filters(df_input, sel_upload_id_val, selected_branches, selected_categories, selected_managers, selected_file_types_val):
    ...
    if 'All' not in selected_file_types_val:
        is_na_report_type = df_filtered['report_type'].isnull()
        is_in_selected_types = df_filtered['report_type'].isin(selected_file_types_val)
        
        if "none" in [str(ft).lower() for ft in selected_file_types_val] or \
           "nan" in [str(ft).lower() for ft in selected_file_types_val] :
            df_filtered = df_filtered[is_in_selected_types | is_na_report_type]
        else: 
            df_filtered = df_filtered[is_in_selected_types & ~is_na_report_type]
...
if primary_date_range and len(primary_date_range) == 2:
    ...
    if 'date' in df_primary_period.columns and pd.api.types.is_datetime64_any_dtype(df_primary_period['date']):
        df_primary_period = df_primary_period[
            (df_primary_period['date'].dt.date >= start_date_filt) &
            (df_primary_period['date'].dt.date <= end_date_filt)
        ]
    else: 
        st.error("Date column issue in df_primary_period, filtering may fail.")
        df_primary_period = pd.DataFrame(columns=df_temp_filtered.columns) 
else: 
    st.error("Primary date range is not set correctly.")
    df_primary_period = pd.DataFrame(columns=df_temp_filtered.columns) 
...
def create_bar_chart(df_source, group_col, title_suffix="", chart_title=None, color_sequence=None):
...
def display_general_dashboard(df_data, figs_container):
    # st.subheader("General Performance Analysis") # Title handled by caller context
    if df_data.empty:
    ...
    if 'date' in df_data.columns and pd.api.types.is_datetime64_any_dtype(df_data['date']) and not df_data['date'].isnull().all():
        ...
        if not trend_data_primary.empty:
            ...
            fig_trend = go.Figure()
            fig_trend.add_trace(go.Bar(x=trend_data_primary['date'], y=trend_data_primary['daily_issues'], name='Daily Issues', marker_color='lightblue',
                                     hovertemplate="<b>%{x|%A, %b %d}</b><br>Issues: %{y}<extra></extra>"))
            fig_trend.add_trace(go.Scatter(x=trend_data_primary['date'], y=trend_data_primary[f'{window_size}-Day MA'], name=f'{window_size}-Day Moving Avg.',
            ...
            fig_trend.update_layout(title_text='Issues Trend (Primary Period - Based on Issue Dates)', xaxis_title='Date', yaxis_title='Number of Issues',
...
def display_complaints_performance_dashboard(df_complaints_raw):
    ...
    with col1:
        df_type_chart_data = df_complaints.explode('Complaint Type')
        fig_type = create_bar_chart(df_type_chart_data, 'Complaint Type', chart_title="Complaints by Type", color_sequence=COMPLAINTS_COLOR_SEQUENCE)
        ...
        if not df_product_chart_source.empty:
            fig_product = create_bar_chart(df_product_chart_source, 'Product Complained About', chart_title="Complaints by Product (Specific Products)", color_sequence=COMPLAINTS_COLOR_SEQUENCE)
    ...
    with col2:
        ...
        if not df_quality_issues_source.empty:
            df_quality_detail_chart_data = df_quality_issues_source.explode('Quality Issue Detail')
            fig_quality_detail = create_bar_chart(df_quality_detail_chart_data, 'Quality Issue Detail', chart_title="Quality Issue Details (for 'جوده' complaints)", color_sequence=COMPLAINTS_COLOR_SEQUENCE)
        ...
        if not df_order_errors_source.empty:
            df_order_error_detail_chart_data = df_order_errors_source.explode('Order Error Detail')
            fig_order_error_detail = create_bar_chart(df_order_error_detail_chart_data, 'Order Error Detail', chart_title="Order Error Details (for 'خطاء فى الطلب' complaints)", color_sequence=COMPLAINTS_COLOR_SEQUENCE)
...
if enable_comparison and comparison_date_range_1 and comparison_date_range_2:
    st.markdown("---")
    st.header("📊 Period Comparison Results (Based on Issue Dates)")

    df_comp1_base = apply_general_filters(df_all_issues, sel_id, sel_branch, sel_cat, sel_am, sel_ft)
    df_comp1 = df_comp1_base[
        (df_comp1_base['date'].dt.date >= comparison_date_range_1[0]) &
        (df_comp1_base['date'].dt.date <= comparison_date_range_1[1])
    ].copy()

    df_comp2_base = apply_general_filters(df_all_issues, sel_id, sel_branch, sel_cat, sel_am, sel_ft)
    df_comp2 = df_comp2_base[
        (df_comp2_base['date'].dt.date >= comparison_date_range_2[0]) &
        (df_comp2_base['date'].dt.date <= comparison_date_range_2[1])
    ].copy()
    ...
    if df_comp1.empty and df_comp2.empty:
        ...
        if not df_comp1.empty:
            is_comp_complaints_data_p1 = (
                (df_comp1['upload_category'] == 'complaints').all() and 
        ...
        if not df_comp2.empty:
            is_comp_complaints_data_p2 = (
                (df_comp2['upload_category'] == 'complaints').all() and 
        ...
        if is_comp_complaints_data_p1 and is_comp_complaints_data_p2 :
            ...
            if not df_combined_complaints_comp.empty:
                df_complaint_type_comp_exploded = df_combined_complaints_comp.explode('Complaint Type').dropna(subset=['Complaint Type'])
                df_complaint_type_comp_exploded = df_complaint_type_comp_exploded[df_complaint_type_comp_exploded['Complaint Type'] != '']
                
                if not df_complaint_type_comp_exploded.empty:
                    complaint_type_comp_data = df_complaint_type_comp_exploded.groupby(['Complaint Type', 'period_label']).size().reset_index(name='count')
                    if not complaint_type_comp_data.empty:
                        fig_complaint_type_comp = px.bar(complaint_type_comp_data, x='Complaint Type', y='count', 
                                                         color='period_label', barmode='group', 
                                                         title='Complaint Types Comparison by Period',
                                                         color_discrete_sequence=COMPLAINTS_COLOR_SEQUENCE) 
                        st.plotly_chart(fig_complaint_type_comp, use_container_width=True)
                    else: st.caption("No complaint type data for comparison chart.")
            ...
            with col_comp1_disp:
                if not df_comp1.empty and 'issues' in df_comp1.columns:
                    df_issues_comp1 = df_comp1[['issues']].copy()
                    df_issues_comp1.dropna(subset=['issues'], inplace=True)
                    ...
                    if not df_issues_comp1.empty:
                        ...
                        # Ensure correct column name after reset_index. Original 'issues_str' becomes 'count'
                        top_issues_df1 = df_issues_comp1['issues_str'].value_counts().nlargest(5).reset_index()
                        top_issues_df1.columns = ['Issue/Violation', 'Count'] # Rename columns explicitly
            ...
            with col_comp2_disp:
                if not df_comp2.empty and 'issues' in df_comp2.columns:
                    df_issues_comp2 = df_comp2[['issues']].copy()
                    df_issues_comp2.dropna(subset=['issues'], inplace=True)
                    ...
                    if not df_issues_comp2.empty:
                        ...
                        top_issues_df2.columns = ['Issue/Violation', 'Count'] # Rename columns explicitly
            ...
            if dfs_to_concat:
                ...
                if not df_combined_branch.empty and 'branch' in df_combined_branch.columns:
                    df_valid_branch_comp = df_combined_branch.copy()
                    df_valid_branch_comp.dropna(subset=['branch'], inplace=True)
                    ...
                    if not df_valid_branch_comp.empty:
                        ...
                        if not branch_comp_data.empty:
                            fig_branch_comp = px.bar(branch_comp_data, x='branch', y='count', color='period_label', 
                                                     barmode='group', title='Issues by Branch (Comparison)')
                            st.plotly_chart(fig_branch_comp, use_container_width=True)
                        else: st.caption("No branch data for comparison chart.")
                    else: st.caption("No valid branch data after filtering for comparison.")
            ...
            if len(period_summary_data) >= 1:
                ...
                fig_period_level_trend = trend_chart_type(df_period_trend, x='Period', y='AverageDailyIssues', text='AverageDailyIssues', 
                                                          title=chart_title_trend, markers=(len(df_period_trend) > 1))
...
is_primary_data_purely_complaints = False
if 'df_primary_period' in locals() and not df_primary_period.empty: # Check if df_primary_period is defined and not empty
    is_primary_data_purely_complaints = (
        (df_primary_period['upload_category'] == 'complaints').all() and
        (df_primary_period['report_type'] == 'performance').all() and
        df_primary_period['upload_category'].nunique() == 1 and
        df_primary_period['report_type'].nunique() == 1
...
if 'df_primary_period' in locals() and not df_primary_period.empty:
    ...
    try:
        ...
        st.sidebar.download_button("Download Primary (CSV)", csv_data_primary, 
                                   f"primary_issues_{primary_date_range[0]:%Y%m%d}-{primary_date_range[1]:%Y%m%d}.csv", 
                                   "text/csv", key="download_csv_primary")
    ...
    try:
        ...
        if is_primary_data_purely_complaints: 
            parsed_details_excel = df_primary_excel_export['issues'].apply(parse_complaint_details)
            df_primary_excel_export = pd.concat([
                df_primary_excel_export.reset_index(drop=True).drop(columns=['issues'], errors='ignore'),
                parsed_details_excel.reset_index(drop=True)
            ], axis=1)
            df_primary_excel_export.rename(columns={
                'Type': 'Complaint Type', 'Product': 'Product Complained About',
                'Quality Detail': 'Quality Issue Detail', 'Order Error': 'Order Error Detail'
            }, inplace=True)
            for col_name in ['Complaint Type', 'Quality Issue Detail', 'Order Error Detail']:
                if col_name in df_primary_excel_export.columns:
                    df_primary_excel_export[col_name] = df_primary_excel_export[col_name].apply(lambda x: ', '.join(x) if isinstance(x, list) and x else (x if not isinstance(x,list) else ''))
        ...
        st.sidebar.download_button(label="Download Primary (Excel)", data=excel_data,
            file_name=f"primary_data_{primary_date_range[0]:%Y%m%d}-{primary_date_range[1]:%Y%m%d}.xlsx",
    ...
    if not is_primary_data_purely_complaints: 
        ...
        if 'pdf_visuals_primary_data' in st.session_state and st.session_state.pdf_visuals_primary_data:
            st.sidebar.download_button(label="Download Visuals PDF (Primary)", data=st.session_state.pdf_visuals_primary_data, 
                                    ...
                                    on_click=lambda: st.session_state.pop('pdf_visuals_primary_data', None)) 
        ...
        if not wk_path or wk_path == "not found": st.sidebar.error("wkhtmltopdf path not set for PDF generation.")
            ...
            with st.spinner("Generating Data Table PDF for Primary Period..."):
                ...
                if is_primary_data_purely_complaints: 
                    ...
                    df_pdf_view_final = pd.concat([
                        df_pdf_view_final.reset_index(drop=True).drop(columns=['issues'], errors='ignore'),
                        parsed_details_pdf.reset_index(drop=True)
                    ], axis=1)
                    ...
                    if 'shift' in df_pdf_view_final.columns and df_pdf_view_final['shift'].notna().any():
                ...
                for col_name in ['Complaint Type', 'Quality Issue Detail', 'Order Error Detail']:
                    if col_name in df_pdf_view_final.columns:
                         df_pdf_view_final[col_name] = df_pdf_view_final[col_name].apply(lambda x: ', '.join(x) if isinstance(x, list) and x else (x if not isinstance(x,list) else ''))
    ...
    if 'pdf_dashboard_primary_data' in st.session_state and st.session_state.pdf_dashboard_primary_data:
        pdf_dl_filename_suffix = "complaints_table" if is_primary_data_purely_complaints else "data_table"
        st.sidebar.download_button(label=f"Download {'Complaints' if is_primary_data_purely_complaints else 'Data'} Table PDF (Primary)", 
                                   data=st.session_state.pdf_dashboard_primary_data, 
                                   file_name=f"{pdf_dl_filename_suffix}_report_primary_{primary_date_range[0]:%Y%m%d}-{primary_date_range[1]:%Y%m%d}.pdf", 
