...
if 'submission_date' not in existing_columns:
    try:
        ...
        conn.commit()
        if 'db_schema_updated_flag' not in st.session_state: # For showing toast once
            st.session_state.db_schema_updated_flag = True
    except sqlite3.OperationalError as e:
        # If this fails, app might not work correctly for the new feature.
        # We'll set a flag to show an error message after set_page_config.
...
if 'db_critical_error_msg' in st.session_state:
    st.error(f"A critical database error occurred during startup: {st.session_state.db_critical_error_msg}. "
             "The 'Effective Date' feature might not work. Please check logs or contact support.")
    del st.session_state.db_critical_error_msg # Show once
...
if 'db_schema_updated_flag' in st.session_state and st.session_state.db_schema_updated_flag:
    ...
    st.session_state.db_schema_updated_flag = False # Clear flag
